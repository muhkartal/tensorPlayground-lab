...
import { 
  Brain, Settings, Sparkles, Play, Pause, Database, Activity, Layers, Cpu, 
  Shuffle, Timer, TrendingUp, Sliders, BarChart3, Box, HelpCircle, ChevronRight,
  ChevronDown, Info, Maximize2, Minimize2, Copy, Download, Upload, Zap,
  GitBranch, Network, Eye, AlertCircle, CheckCircle, Loader, Terminal,
  Plus, Minus, X
} from 'lucide-react';
...
const NeuralNetworkVisualizer = () => {
    ...
    try {
      ...
      setConsoleMessages(prev => [
        ...prev,
        {
          ...
          message: `Generated ${type} dataset with ${data.length} samples (${Math.round((1-testRatio)*100)}% train, ${Math.round(testRatio*100)}% test)`,
          type: 'info'
        }
      ]);
  ...
  const createModel = () => {
    ...
    for (let i = 0; i < layers.length - 1; i++) {
      model.add(tf.layers.dense({
        units: layers[i + 1],
        activation: i === layers.length - 2 && selectedDataset !== 'sine' ? 'sigmoid' : activationFunction,
        inputShape: i === 0 ? [layers[i]] : undefined,
        kernelInitializer: getInitializer(),
        biasInitializer: 'zeros',
        kernelRegularizer: regularizationType !== 'none' ? 
          tf.regularizers.l2({ l2: regularizationRate }) : null
      }));
  ...
  const visualizeNetwork = () => {
    ...
    for (let i = 0; i < layers.length; i++) {
      const x = margin + i * layerWidth;
      let label = i === 0 ? `Input (${layers[i]})` : 
                  i === layers.length - 1 ? `Output (${layers[i]})` : 
                  `Hidden ${i} (${layers[i]})`;
      ...
      for (let fromNeuron = 0; fromNeuron < layers[fromLayer]; fromNeuron++) {
        for (let toNeuron = 0; toNeuron < layers[toLayer]; toNeuron++) {
          ...
          connectionsGroup.append('line')
            ...
            .attr('class', `connection layer-${fromLayer}-${fromNeuron} layer-${toLayer}-${toNeuron}`)
            .attr('x1', fromPos.x)
            .attr('y1', fromPos.y)
            .attr('x2', toPos.x)
            .attr('y2', toPos.y)
    ...
    for (let i = 0; i < layers.length; i++) {
      ...
      for (let j = 0; j < layers[i]; j++) {
        ...
        const neuronGroup = layerGroup.append('g')
  ...
  const highlightNeuron = (layer, neuron) => {
      ...
      .attr('fill', function() {
        ...
        if (match) {
          ...
          if (activations[l] && activations[l][n] !== undefined) {
            ...
            const maxAct = d3.max(activations.flat().filter(a => a !== undefined).map(Math.abs)) || 0.1;
    ...
    // Highlight selected neuron
    svg.select(`.neuron.layer-${layer}.neuron-${neuron}`)
    ...
    svg.selectAll(`.connection.layer-${layer}-${neuron}`)
      .attr('stroke', '#22c55e')
      .attr('stroke-width', 3)
    ...
    if (activations[layer] && activations[layer][neuron] !== undefined) {
      setConsoleMessages(prev => [
        ...
        {
          timestamp: new Date().toLocaleTimeString(),
          message: `Selected neuron: Layer ${layer} Neuron ${neuron}, Activation = ${activations[layer][neuron].toFixed(4)}`,
          type: 'info'
        }
      ]);
    }
  ...
  const runTrainingStep = async () => {
    ...
    try {
      ...
      if (!trainDataCache || !trainDataCache.xs) {
        ...
        if (!trainDataCache || !trainDataCache.xs) {
          handleTrainingError("Could not generate training data", new Error("Data generation failed"));
          return;
        }
      }
      ...
      if (testDataCache && testDataCache.xs) {
        try {
          const evalResult = await modelRef.current.evaluate(
            testDataCache.xs, 
            testDataCache.ys, 
            { verbose: 0 }
          );
      ...
      if (iterations % 10 === 0) {
        ...
        setConsoleMessages(prev => [
          ...
          {
            timestamp: new Date().toLocaleTimeString(),
            message: `Iteration ${iterations}: Training Loss = ${latestLoss.toFixed(4)}, Test Loss = ${loss.toFixed(4)}`,
            type: 'info'
          }
        ]);
      }
      ...
      if (iterations < 1000 && trainingControlRef.current) {
        ...
        setConsoleMessages(prev => [
          ...
          {
            timestamp: new Date().toLocaleTimeString(),
            message: `Training completed after ${iterations} iterations. Final test loss: ${loss.toFixed(4)}`,
            type: 'success'
          }
        ]);
      }
            ...
            {activeTab === 'architecture' && (
              <div>
                <div style={styles.section}>
                  ...
                  >
                    <div style={styles.sectionTitle}>
                      <GitBranch size={16} color={expandedSection === 'layers' ? '#3b82f6' : '#9ca3af'} />
                      <span>Network Layers</span>
                    </div>
                    {expandedSection === 'layers' ? <ChevronDown size={16} color="#3b82f6" /> : <ChevronRight size={16} />}
                  </div>
                            ...
                            <React.Fragment key={idx}>
                              <div style={styles.layerInputWrapper}>
                                ...
                                <input
                                  ...
                                  style={{
                                    ...styles.layerInput,
                                    borderColor: selectedNeuron?.layer === idx ? '#3b82f6' : 'rgba(255, 255, 255, 0.08)',
                                    background: selectedNeuron?.layer === idx ? 'rgba(59, 130, 246, 0.08)' : 'rgba(13, 13, 20, 0.6)',
                                    boxShadow: selectedNeuron?.layer === idx ? '0 0 10px rgba(59, 130, 246, 0.3)' : '0 2px 4px rgba(0, 0, 0, 0.2)'
                                  }}
                                  min="1"
                                  disabled={isTraining}
                ...
                <div style={styles.section}>
                  <div 
                    style={styles.sectionHeader}
                    onClick={() => setExpandedSection(expandedSection === 'activation' ? '' : 'activation')}
                  >
                    <div style={styles.sectionTitle}>
                      <Zap size={16} color={expandedSection === 'activation' ? '#3b82f6' : '#9ca3af'} />
                      <span>Activation Function</span>
                    </div>
                    {expandedSection === 'activation' ? <ChevronDown size={16} color="#3b82f6" /> : <ChevronRight size={16} />}
                  </div>
                ...
                <div style={styles.section}>
                  ...
                  >
                    <div style={styles.sectionTitle}>
                      <Database size={16} color={expandedSection === 'dataset' ? '#3b82f6' : '#9ca3af'} />
                      <span>Dataset</span>
                    </div>
                    {expandedSection === 'dataset' ? <ChevronDown size={16} color="#3b82f6" /> : <ChevronRight size={16} />}
                  </div>
                    ...
                    <div style={styles.sectionContent}>
                      ...
                      <div style={styles.formGroup}>
                        <label style={styles.label}>
                          ...
                          <span>Noise Level: <span style={styles.sliderValue}>{noiseLevel}%</span></span>
                        </label>
                        <div style={{ position: 'relative', padding: '4px 0' }}>
                          <input
                            ...
                            style={{
                              ...styles.slider,
                              background: `linear-gradient(to right, #60a5fa ${noiseLevel * 2}%, rgba(13, 13, 20, 0.6) ${noiseLevel * 2}%)`,
                            }}
                            disabled={isTraining}
                          />
                        </div>
                      ...
                      <div style={styles.formGroup}>
                        <label style={styles.label}>
                          ...
                          <span>Test Ratio: <span style={styles.sliderValue}>{Math.round(testRatio * 100)}%</span></span>
                        </label>
                        <div style={{ position: 'relative', padding: '4px 0' }}>
                          <input
                            type="range"
                            ...
                            style={{
                              ...styles.slider,
                              background: `linear-gradient(to right, #60a5fa ${testRatio * 200}%, rgba(13, 13, 20, 0.6) ${testRatio * 200}%)`,
                            }}
                            disabled={isTraining}
                          />
                        </div>
            ...
            {activeTab === 'training' && (
              <div>
                <div style={styles.section}>
                  <div 
                    style={styles.sectionHeader}
                    onClick={() => setExpandedSection(expandedSection === 'optimizer' ? '' : 'optimizer')}
                  >
                    <div style={styles.sectionTitle}>
                      <TrendingUp size={14} />
                      <span>Optimizer</span>
                    </div>
                    {expandedSection === 'optimizer' ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                  </div>
            ...
            {activeTab === 'advanced' && (
              <div>
                <div style={styles.section}>
                  <div 
                    style={styles.sectionHeader}
                    onClick={() => setExpandedSection(expandedSection === 'regularization' ? '' : 'regularization')}
                  >
                    <div style={styles.sectionTitle}>
                      <Sliders size={14} />
                      <span>Regularization</span>
                    </div>
                    {expandedSection === 'regularization' ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                  </div>
                ...
                <div style={styles.section}>
                  <div 
                    style={styles.sectionHeader}
                    onClick={() => setExpandedSection(expandedSection === 'initialization' ? '' : 'initialization')}
                  >
                    <div style={styles.sectionTitle}>
                      <Box size={14} />
                      <span>Weight Initialization</span>
                    </div>
                    {expandedSection === 'initialization' ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                  </div>
          ...
          <div style={styles.metricsBar}>
            <div style={styles.metricCard}>
              ...
              <div style={{...styles.metricChange, ...styles.metricNegative}}>
                <TrendingUp size={12} />
                {lossHistory.length > 1 ? ((lossHistory[lossHistory.length-1] - lossHistory[lossHistory.length-2]) * 100).toFixed(2) : '0.00'}%
              </div>
            </div>
            
            <div style={styles.metricCard}>
              <div style={styles.metricLabel}>Training Loss</div>
              <div style={styles.metricValue}>{lossHistory.length > 0 ? lossHistory[lossHistory.length-1].toFixed(4) : '0.000'}</div>
              <div style={{...styles.metricChange, ...styles.metricNegative}}>
                <TrendingUp size={12} />
                {lossHistory.length > 1 ? 'Δ' + (lossHistory[lossHistory.length-1] - lossHistory[lossHistory.length-2]).toFixed(4) : 'N/A'}
              </div>
            </div>
            ...
            <div style={styles.metricCard}>
              ...
              <div style={styles.metricChange}>
                {isTraining && <Loader size={12} style={{ animation: 'spin 2s linear infinite' }} />}
                ...
                <div key={idx} style={styles.consoleLine}>
                  ...
                  <span style={{ 
                    color: msg.type === 'error' ? '#ef4444' : 
                           ...
                           msg.type === 'warning' ? '#f59e0b' : '#e5e7eb' 
                  }}>
                    {msg.message}
                  </span>
                </div>
          ...
          <div style={styles.helpContent}>
            <div style={styles.helpSection}>
              ...
              <div style={styles.helpText}>
                This application provides an interactive, production-grade neural network training environment powered by TensorFlow.js. 
                It allows you to configure, train, and visualize deep neural networks in real-time directly in your browser, 
                offering a transparent view into the internal mechanics of how neural networks learn.
              </div>
            </div>
            ...
            <div style={styles.helpSection}>
              ...
              <div style={styles.helpText}>
                The entire training process is handled by TensorFlow.js with no simplified abstractions:
                <ul style={{marginTop: '8px', paddingLeft: '20px'}}>
                  ...
                  <li>Support for various activation functions, optimizers, and regularization techniques</li>
            ...
            <div style={styles.helpSection}>
              ...
              <div style={styles.helpText}>
                The visualization displays the actual state of neurons and connections during training:
                <ul style={{marginTop: '8px', paddingLeft: '20px'}}>
                  ...
                  <li>Blue connections represent positive weights, red connections represent negative weights</li>
                  <li>All visualized metrics are directly extracted from the TensorFlow.js model</li>
                  <li>Visualization updates in real-time during training to show the learning process</li>
                </ul>
              </div>
            </div>
            ...
            <div style={styles.helpSection}>
              ...
              <div style={styles.helpText}>
                <p style={{marginBottom: '8px'}}>The application provides several classic machine learning problems:</p>
                <table style={{width: '100%', borderCollapse: 'collapse'}}>
                  <tbody>
                    <tr style={{borderBottom: '1px solid rgba(255,255,255,0.1)'}}>
                      <td style={{padding: '8px', fontWeight: 'bold'}}>XOR:</td>
                      <td style={{padding: '8px'}}>The classical logical XOR problem that requires a hidden layer to solve. Demonstrates the importance of non-linear transformations in neural networks.</td>
                    </tr>
                    <tr style={{borderBottom: '1px solid rgba(255,255,255,0.1)'}}>
                      <td style={{padding: '8px', fontWeight: 'bold'}}>Circle:</td>
                      <td style={{padding: '8px'}}>Binary classification of points inside or outside a circle. Demonstrates the network's ability to learn circular decision boundaries.</td>
                    </tr>
                    <tr style={{borderBottom: '1px solid rgba(255,255,255,0.1)'}}>
                      <td style={{padding: '8px', fontWeight: 'bold'}}>Gaussian:</td>
                      <td style={{padding: '8px'}}>Classification of points from two Gaussian distributions. Shows how networks handle overlapping distributions and probability densities.</td>
                    </tr>
                    <tr style={{borderBottom: '1px solid rgba(255,255,255,0.1)'}}>
                      <td style={{padding: '8px', fontWeight: 'bold'}}>Spiral:</td>
                      <td style={{padding: '8px'}}>Classification of points from intertwined spiral patterns. Demonstrates the power of neural networks to learn complex, non-linear decision boundaries.</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div style={styles.helpSection}>
              ...
              <div style={styles.helpText}>
                ...
                <ul style={{marginTop: '8px', paddingLeft: '20px'}}>
                  <li><strong>Architecture:</strong> Add, remove, and resize layers to create custom network topologies</li>
                  <li><strong>Activation Functions:</strong> Choose from ReLU, Sigmoid, Tanh, and Linear activations</li>
                  <li><strong>Optimizers:</strong> SGD with momentum, Adam, and RMSProp with configurable learning rates</li>
                  <li><strong>Regularization:</strong> L1, L2, and Dropout options to prevent overfitting</li>
                  <li><strong>Weight Initialization:</strong> Xavier, He, and Random initialization strategies</li>
                  <li><strong>Training Parameters:</strong> Batch size, noise level, and test/train ratio</li>
                </ul>
              </div>
            </div>
          </div>
